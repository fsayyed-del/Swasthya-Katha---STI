...
import { Sparkles, MoveRight, Camera } from 'lucide-react';
...
export const CoverShell: React.FC<CoverShellProps> = ({
  ...
  return (
    <div
      onClick={onOpen}
      className={`relative w-full h-full p-4 sm:p-7 lg:p-8 flex flex-col justify-between select-none cursor-pointer bg-gradient-to-br from-[#FAF5EC] via-[#F4EDE0] to-[#EAE0CD] border-r-4 border-brass/40 shadow-2xl rounded-r-3xl overflow-hidden ${className}`}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="absolute inset-3 border border-brass/25 rounded-2xl pointer-events-none" />

      {/* Top Header Badge & Language Switcher */}
      <div className="relative z-10 flex items-center justify-between">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-mineral-green/15 border border-mineral-green/30 rounded-full text-[10px] sm:text-xs font-bold text-mineral-green-dark uppercase tracking-widest">
      ...
      <div className="relative z-10 text-center my-auto space-y-3 sm:space-y-4">
        {/* Paper-Cut Mineral-Green Protective Hand Motif */}
        <div className="w-16 h-16 sm:w-22 sm:h-22 mx-auto rounded-3xl bg-gradient-to-br from-[#4F7A6C] to-[#3B5C51] text-paper flex items-center justify-center shadow-lg border-2 border-brass/60 p-2.5">
        ...
        <p className="text-sm sm:text-base text-ink-muted font-body font-medium max-w-sm mx-auto leading-relaxed">
          {isHindi
        ...
        <div className="pt-1">
          <span className="inline-block px-3 py-1 bg-care-blue/15 border border-care-blue/30 rounded-full text-[11px] sm:text-xs font-bold text-care-blue-dark">
      ...
      <div className="relative z-10 text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-5 py-2 bg-ink-teal hover:bg-teal-dark text-paper font-semibold text-xs sm:text-sm rounded-full transition-all shadow-lg hover:scale-105 active:scale-95 animate-pulse">
